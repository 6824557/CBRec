...
parser.add_argument('--oriDataset',  default='PH')
parser.add_argument('--saveDir',  default='PHPro')
...
parser.add_argument('--distence',  default=5, type=int )
args = parser.parse_args()
print(args.oriDataset)
args.saveDir = args.saveDir+"Glo"
print(args.saveDir)
...
cate = data_train['venueCategory'].unique()
#id映射为0到全部
...
data_train['cid'] = data_train['venueCategory'].map(cate_to_id)
...
data_train = data_train[['uid','pid','cid','venueCategory','latitude','longitude','time_slot','Timestamp']]

columns = ['uid','pid','cid','venueCategory','latitude','longitude','time_slot','Timestamp']
...
for slot_id in train_slots:
    ...
    for location1 in dis.keys():
        nearby_locations = [location2 for location2, distance in dis[location1].items() if distance < args.distence]
...
pop_item = [x / (total+n_item) for x in pop_item]
pop_item = np.array(pop_item)
pop_item = (pop_item - pop_item.min()) / (pop_item.max() - pop_item.min())

with open("./"+path_folder+"/item_pop_seq_ori.txt","w") as f:
    for i in range(n_item):
        f.write(str(i+1)+' '+ str(pop_item[i])+ '\n')
...
for i in train_slots:
    ...
    for item,pop in item_pop_list_t:
        ...
        pop_item[item-1] += pop
        # pop_item[i][item] = 1e6*(pop)/(total)
pop_item = [x / (total+n_cate) for x in pop_item]
pop_item = np.array(pop_item)
pop_item = (pop_item - pop_item.min()) / (pop_item.max() - pop_item.min())
